...
const RuleCard: React.FC<{ rule: BatteryRule }> = ({ rule }) => {
    ...
    <Card style={{ padding: 0, overflow: 'hidden' }}>
      ...
      >
        ...
        <div style={{
          ...
          fontSize: 18, color: rule.enabled ? 'var(--accent)' : 'var(--text-muted)',
          flexShrink: 0,
        }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          />
          <span style={{ fontSize: 12, color: 'var(--text-muted)', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', display: 'inline-block' }}>▾</span>
        </div>
      </div>
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{ paddingTop: 16, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500, marginBottom: 8 }}>Action to perform</div>
              <ActionPicker
                value={rule.action}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <Btn size="sm" variant="danger" onClick={() => removeBatteryRule(rule.id)}>
                Delete Rule
              </Btn>
            </div>
          </div>
...
const BatteryMeter: React.FC = () => {
  ...
  const color = !hasBattery
    ? 'var(--text-muted)'
    : pct <= 10 ? 'var(--danger)' : pct <= 25 ? 'var(--warn)' : pct <= 50 ? 'var(--accent)' : 'var(--success)';
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        <div style={{ position: 'relative', flexShrink: 0 }}>
          <svg width={80} height={140}>
            ...
            {/* Body */}
            <rect x={5} y={8} width={70} height={126} rx={8} fill="var(--bg-overlay)" stroke="var(--border)" strokeWidth={2} />
            {/* Fill */}
            <rect
              x={9} y={12 + (1 - (hasBattery ? pct : 0) / 100) * 118}
              width={62} height={(hasBattery ? pct : 0) / 100 * 118}
              rx={4} fill={color}
              style={{ transition: 'all 1s ease', filter: `drop-shadow(0 0 8px ${color}60)` }}
            />
            {/* % text */}
            <text x={40} y={76} textAnchor="middle" fontSize={16} fontFamily="var(--font-mono)"
              fontWeight="700" fill="white" style={{ textShadow: '0 1px 4px rgba(0,0,0,0.8)' }}>
              {hasBattery ? `${pct}%` : 'N/A'}
            </text>
            {battery.charging && (
              <text x={40} y={96} textAnchor="middle" fontSize={18} fill="white">⚡</text>
            )}
          </svg>
        </div>
        ...
        {/* Info */}
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 36, fontFamily: 'var(--font-mono)', fontWeight: 700, color, lineHeight: 1 }}>
            {hasBattery ? (
              <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 16 }}>
            {[
              { label: 'Status',  value: !hasBattery ? 'N/A' : battery.charging ? 'Charging' : onAc ? 'Plugged' : 'Discharging' },
      ...
      <div style={{ marginTop: 24 }}>
        <div style={{
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)' }}>
            RULES ({batteryRules.filter(r => r.enabled).length} active)
          </span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {batteryRules.map((r: BatteryRule) => <RuleCard key={r.id} rule={r} />)}
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 12, color: 'var(--info)', fontWeight: 600, marginBottom: 6 }}>
            ℹ Battery monitoring notes
          </div>
          <ul style={{ fontSize: 11, color: 'var(--text-muted)', display: 'flex', flexDirection: 'column', gap: 4, listStyle: 'disc', paddingLeft: 16 }}>
            <li>Rules only apply to laptops or devices with batteries.</li>
            <li>"Low Battery" uses the OS threshold (~15–20% depending on Windows settings).</li>
            <li>Multiple rules can be active simultaneously.</li>
            <li>Actions execute in priority order: shutdown &gt; restart &gt; hibernate &gt; sleep &gt; lock.</li>
          </ul>
        </div>
      </div>
    </div>
